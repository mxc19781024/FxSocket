...
CFuzzyCompareHelper::CFuzzyCompareHelper(CFuzzyCompare *pFuzzyCompare)
	:m_pFuzzyCompare(pFuzzyCompare)
{
}

CFuzzyCompareHelper::~CFuzzyCompareHelper()
{
}

/**
* @brief 模糊匹配，返回值为bool类型，true为匹配成功，false为匹配失败
* @note
* @param strInput[in]传入搜索时的文本内容
* @param strOrigin[int] 传入要搜索的文本的原始内容
* @return
*/
bool CFuzzyCompareHelper::fuzzyCompare(const QString &strInput, const QString &strOrigin)
{
	QString strItemInitial;		//当前节点文本的拼音首字母
	QString strItemQuanPin;		//当前节点文本的全拼
	getPinYin(strOrigin, strItemInitial, strItemQuanPin);
	QString strStart = "^";
	QRegExp rx(strStart.append(strItemQuanPin));
	QRegExpValidator v(rx, 0);
	int pos = 0;
	//如果输入的内容匹配当前文本
	if (strItemInitial.contains(strInput, Qt::CaseInsensitive) || v.validate(const_cast<QString &>(strInput), pos) != QValidator::Invalid
		|| strOrigin.contains(strInput))
	{
		return true;
	}
	return false;
...
* @return
*/
void CFuzzyCompareHelper::getPinYin(const QString &strOrigin, QString &strInitial, QString &strQuanPin)
{
	// 根据指定的汉字字符串获取到拼音首字母
	std::string strTmp = getFirstLetter(strOrigin.toLocal8Bit().data());
	strInitial = strTmp.c_str();

	// 根据指定的汉字字符串获取到汉字全拼
	std::vector<std::pair<bool, std::vector<std::string>>> vec = toPinyinFromUtf8(strOrigin.toUtf8().data(), false);
	std::string strContent;
	for (auto iter = vec.begin(); iter != vec.end(); ++iter)
	{
		bool bSuccess = iter->first;
		// 返回汉字的拼音，vector.size>1代表文字有多音字
		std::vector<std::string> pairVec = iter->second;

		// 取汉字的拼音的第一个
		strContent = strContent + pairVec.front();
		// 		for (auto it = pairVec.begin(); it != pairVec.end(); ++it)
		// 		{
		// 			// 将单个词语的多音词拼在一起
		// 			strContent = strContent + *it;
		// 		}
	}
	strQuanPin = strContent.c_str();
}
...
* @return 返回汉字词组的首字母拼音
*/
std::string CFuzzyCompareHelper::getFirstLetter(const char* strChs)
{
...
* @return
*/
bool CFuzzyCompareHelper::isHanziUnicode(const Unicode unicode) const
{
...
*/
std::vector<std::string> CFuzzyCompareHelper::toPinyinFromUnicode(const Unicode hanzi_unicode, const bool with_tone /*= true*/) const
{
	...
	while ('\0' != *p) {
		if (split_c == *p) {
			polyphone_list_with_tone.emplace_back(string(p_old, p));
			p_old = p + 1;
		}
		++p;
	}
	...
	for (auto iter_const = polyphone_list_with_tone.cbegin(); iter_const != polyphone_list_with_tone.cend(); ++iter_const)
	{
		const string &polyphone = *iter_const;
		const auto& iter = find_if(polyphone.cbegin(), polyphone.cend(), isdigit);
		polyphone_list_without_tone.emplace_back(string(polyphone.cbegin(), iter));
	}
...
* @return
*/
bool CFuzzyCompareHelper::isUtf8(const std::string& s) const
{
...
* @return
*/
std::vector<std::pair<bool, std::vector<std::string>>> CFuzzyCompareHelper::toPinyinFromUtf8(const std::string& s, const bool with_tone /*= true*/, const bool replace_unknown /*= false*/, const std::string& replace_unknown_with /*= ""*/) const
{
	...
	for (auto iter = s.cbegin(); iter != s.cend(); NULL) {
		...
		if (!ok) {
			if (replace_unknown) {
				...
				pinyin_list.emplace_back(string(iter_old, iter));
			}
		}
...
const char* CFuzzyCompareHelper::pinyin_list_with_tone_[] = {
	"yi1",
	"ding1",
	"kao3",
	"qi1",
...
const CFuzzyCompareHelper::Unicode CFuzzyCompareHelper::begin_hanzi_unicode_ = 0X4E00;
const CFuzzyCompareHelper::Unicode CFuzzyCompareHelper::end_hanzi_unicode_ = 0X9FA5;