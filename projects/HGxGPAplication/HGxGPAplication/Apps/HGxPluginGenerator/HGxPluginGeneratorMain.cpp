...
int main(int argv, char** argc)
{
    ...
    qApp->setApplicationName("CGxPluginGenerator");

    // init global template defaults
    QSettings settings;
    if (!settings.contains(CGxPluginGeneratorConstants::PLUGIN_LICENSE_MARKER))
    {
        ...
        settings.setValue(CGxPluginGeneratorConstants::PLUGIN_LICENSE_MARKER, licenseText.arg(organization));
    }
    ...
    //   dir.cdUp();
    //   QString strPath = dir.path() + "/plugins/Debug";

    //   qApp->addLibraryPath(strPath);

    //   QStringList libFilter;
    //   libFilter << "*.dll" << "*.so" << "*.dylib";
    //   QDirIterator dirIter(strPath, libFilter, QDir::Files);
    //   while(dirIter.hasNext())
    //   {
    //     try
    //     {
    //       QString fileLocation = dirIter.next();
    //       if (fileLocation.contains("com_henggetec_plugingenerator_ui"))
    //       {
    ...
    CGxPluginGenerator generator(framework.data());
    generator.show();
