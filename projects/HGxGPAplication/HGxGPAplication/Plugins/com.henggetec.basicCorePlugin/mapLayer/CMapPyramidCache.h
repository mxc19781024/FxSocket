...
class CMapPyramidCacheWgt;

class CMapPyramidCache : public QObject
{
	Q_OBJECT
public:

	static CMapPyramidCache *getInstance(QWidget *parent = NULL)
	{
		if (!s_pRyCacheInstance)
		{
			s_pRyCacheInstance = new CMapPyramidCache(parent);
		}
...
private:
	CMapPyramidCache(QWidget *parent = NULL);
	~CMapPyramidCache();

private:
	// 金字塔缓存单例
	static CMapPyramidCache			*s_pRyCacheInstance;

	CMapPyramidCacheWgt				*m_pRyCacheWgt;
	...
	public:
		~CGarbo()
		{
			if (CMapPyramidCache::s_pRyCacheInstance)
			{
				delete CMapPyramidCache::s_pRyCacheInstance;
			}
		}
	};
