...
#include "earthLayer/LayerProperty/CEarthFacetLayerPropertyWgt.h"
...
CEarthFacetLayerPropertyWgt::CEarthFacetLayerPropertyWgt(QWidget *parent)
	: QWidget(parent)
{
	ui.setupUi(this);
...
CEarthFacetLayerPropertyWgt::~CEarthFacetLayerPropertyWgt()
{

}

void CEarthFacetLayerPropertyWgt::setCurrentLayer(HGxScene::CGxGlobalLayer* pLayer)
{
	...
	else
	{
 	...
 	QColor fillColor = QColor::fromRgbF(osgFillColor.r(), osgFillColor.g(), osgFillColor.b()/*, osgFillColor.a()*/);
...
HGxScene::CGxGlobalLayer* CEarthFacetLayerPropertyWgt::getCurrentLayer()
{
	return m_pModelLayer;
}

//初始化信号和槽函数
void CEarthFacetLayerPropertyWgt::initSignalAndSlots()
{
...
HGxScene::CGxSceneBase* CEarthFacetLayerPropertyWgt::getCurrentScene()
{
...
void CEarthFacetLayerPropertyWgt::layerNameChangedSlot(const QString &strLayerName)
{
	m_pModelLayer->setName(strLayerName.toUtf8().data());
	emit layerNameChangedSignal(strLayerName);
}

void CEarthFacetLayerPropertyWgt::dataPathChangedSlot(const QString &path )
{

}

void CEarthFacetLayerPropertyWgt::allViewScopeChangedSlot(bool bChecked)
{
	if (bChecked)
	{
...
void CEarthFacetLayerPropertyWgt::viewScopeChangedSlot()
{
...
void CEarthFacetLayerPropertyWgt::lineColorChangedSlot(const QColor &lineColor)
{
...
void CEarthFacetLayerPropertyWgt::lineTranChangedSlot(int iTranparency)
{
	QColor curColor = ui.pLineColorWgt->getCurrentColor();
	double dfTran = ui.pLineOpacitySlider->value() * 1.0 / 100;

	osg::Vec4f fillColor(curColor.redF(), curColor.greenF(), curColor.blueF(), dfTran);
	m_pModelLayer->setLineColor(fillColor);
}

void CEarthFacetLayerPropertyWgt::lineWidthChangedSlot(int iLineWidth)
{
	m_pModelLayer->setLineWidth(iLineWidth);
}

void CEarthFacetLayerPropertyWgt::lineTypeChangedSlot(int iLineType)
{
	m_pModelLayer->setLineStipple(ui.pLineTypeCBBox->lineStyle());
}

void CEarthFacetLayerPropertyWgt::fillColorChangedSlot(const QColor &color )
{
...
void CEarthFacetLayerPropertyWgt::fillTranChangedSlot(int iTransparency)
{
	QColor curColor = ui.pFillColorWgt->getCurrentColor();
	double dfTran = ui.pFillOpacitySlider->value() * 1.0 / 100;
...
void CEarthFacetLayerPropertyWgt::getCurrentViewScopeSlot()
{
