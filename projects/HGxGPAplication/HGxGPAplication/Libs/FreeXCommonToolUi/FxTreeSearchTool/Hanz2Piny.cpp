...
bool Hanz2Piny::isHanziUnicode (const Unicode unicode) const
{
    return((begin_hanzi_unicode_ <= unicode) && (unicode <= end_hanzi_unicode_));
}

// 基于 unicode 查找对应汉字的拼音，若为多音字则可能返回多个拼音
vector<string> Hanz2Piny::toPinyinFromUnicode (const Unicode hanzi_unicode, const bool with_tone) const
{
    ...
    while ('\0' != *p) {
        if (split_c == *p) {
            polyphone_list_with_tone.emplace_back(std::string(p_old, p));
            p_old = p + 1;
        }
        ++p;
    }
...
bool Hanz2Piny::isUtf8 (const string& s) const
{
    return(utf8::is_valid(s.cbegin(), s.cend()));
}


vector<vector<string>> Hanz2Piny::toPinyinFromUtf8(const std::string &s) const
{
	vector<vector<string>> pinyin_list_list;
	...
	for (auto iter = s.cbegin(); iter != s.cend(); NULL)
	{
		...
		pinyin_list = toPinyinFromUnicode(unicode, false);
		...
		pinyin_list_list.push_back(pinyin_list);
	}
