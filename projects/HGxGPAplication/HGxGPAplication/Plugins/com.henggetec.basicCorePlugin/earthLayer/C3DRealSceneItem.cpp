...
C3DRealSceneItem::C3DRealSceneItem(QWidget *parent, bool bIsCheckable, const QString &name, const QPixmap &normalPixmap, const QPixmap &hoverPixmap, const QPixmap &selectedPixmap)
	: QWidget(parent)
...
C3DRealSceneItem::~C3DRealSceneItem()
{

}

void C3DRealSceneItem::setSelected(bool bSelected)
{
...
void C3DRealSceneItem::paintEvent(QPaintEvent *event)
{
...
void C3DRealSceneItem::mouseReleaseEvent(QMouseEvent *event)
{
...
void C3DRealSceneItem::mousePressEvent(QMouseEvent *event)
{
	if (event->button() == Qt::LeftButton)
	{
		if(!m_bIsCheckable)
...
void C3DRealSceneItem::enterEvent(QEvent *event)
{
	m_bMouseHover = true;
	update();
...
void C3DRealSceneItem::leaveEvent(QEvent *event)
{
	m_bMouseHover = false;
	update();
