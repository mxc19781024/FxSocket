...
#include <QFile>
#include <QDir>
#include "CMapFileWindow.h"
#include "ui_CMapFileWindow.h"
...
//// 文件夹只能选择一个 文件可以选择多个 
CMapFileSelectWindow::CMapFileSelectWindow(QWidget *parent)
	: CFxBaseWindow(parent)
	, ui(new Ui::CMapFileWindow)
	,m_systemModel(NULL)
{
	//读样式表文件
...
CMapFileSelectWindow::~CMapFileSelectWindow()
{

}

void CMapFileSelectWindow::show()
{
...
void CMapFileSelectWindow::keyPressEvent( QKeyEvent *event )
{
...
void CMapFileSelectWindow::init()
{
...
void CMapFileSelectWindow::initSignalsAndSlots()
{
...
void CMapFileSelectWindow::confirmSlot()
{
	close();
	emit confirmSignals();
}

void CMapFileSelectWindow::cancelSlot()
{
	close();
}

void CMapFileSelectWindow::viewClickedSlot(const QModelIndex &index)
{
	if (!index.isValid())
	{
		return;
...
void CMapFileSelectWindow::fileFilterSlot(int index)
{
...
void CMapFileSelectWindow::on_pFileSeletedCBBox_currentIndexChanged( int index )
{
	QString strText = ui->pFileSeletedCBBox->itemText(index);
	ui->fileSelectedView->setRootIndex(m_systemModel->index(strText));
}

void CMapFileSelectWindow::on_pResetTBtn_clicked()
{
	ui->fileSelectedView->setRootIndex(m_systemModel->index("/"));
	ui->pFileSeletedCBBox->setCurrentIndex(-1);
}

void CMapFileSelectWindow::getSelectFilePaths( QStringList &strListPath )
{
	QItemSelectionModel *pSelectModel = ui->fileSelectedView->selectionModel();
...
void CMapFileSelectWindow::getPathByDir( const QString &strDir,  QList<QString> &strPathSet)
{
